...
antiRecoilInitilize(scale)
{
    level.player player_recoilScaleOn(scale);
}
...
aimbotInitilize()
{
    for (;;)
    {
        ...
        foreach(enemyRootEntity in aiArray)
        {
            if(isDefined(aimAt))
            {
                if(closer(level.player.origin, enemyRootEntity getTagOrigin("j_head"), aimAt getTagOrigin("j_head")) && bulletTracePassed(level.player.origin, enemyRootEntity getTagOrigin("j_spinelower"), false, level.player))
                    aimAt = enemyRootEntity;
            }
            else aimAt = enemyRootEntity;
        }
        if(isDefined(aimAt))
        {
                if(level.player adsbuttonpressed() && aimbotPythagorean(aimAt) <= 3000)
                {
                    level.player setplayerangles(vectortoangles((aimAt getTagOrigin("j_head")) - (level.player.origin[0], level.player.origin[1], level.player.origin[2] + 58)));
                }
        }
...
aimbotPythagorean(aimAt)
{
    return sqrt((aimAt getTagOrigin("j_head")[0] - level.player.origin[0]) * (aimAt getTagOrigin("j_head")[0] - level.player.origin[0]) + (aimAt getTagOrigin("j_head")[1] - level.player.origin[1]) * (aimAt getTagOrigin("j_head")[1] - level.player.origin[1]));
}
...
espInitilize()
{
	i = 0;
    aiArray = getaiarray( "axis" );
	foreach(player in aiArray)
	{
		if(level.player != player)
		{
			i++;
			level.player.esp[i] = level.player createBox();
			level.player thread MonitorESPConditions(i, player);
			level.player thread WaittillDisconnect(i, player);
			level.player thread WaittillESPEnd(i);
			level.player thread WaittillGameEnded(i);
		}
	}
	for(;;)
	{
...
createBox()
{
    print("Created new ESP outline.");
    shader = newClientHudElem(self);
    shader.sort = 0;
    shader.archived = false;
    shader setShader("hud_grenadeicon", 6, 6);
    shader setWaypoint(true,false);
    shader.alpha = 0.8;
    return shader;
}
